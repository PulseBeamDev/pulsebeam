...
//! allocations. See `crates/pulsebeam/docs/thread-per-core.md`.
#![allow(
    clippy::disallowed_types,
    reason = "the handles are Arc<AtomicU64> because metrics::Counter::from_arc demands Send + Sync. Each slot is registered, written and read by exactly one shard on one core, so nothing is shared between cores and nothing contends. See crates/pulsebeam/docs/thread-per-core.md."
)]
