...
This crate preserves the former API for the CLI while its migration finishes.
Its implementation now lives in `agents/pulsebeam-agent-native`; do
...
Run `cargo test -p pulsebeam-agent` for focused compatibility work. The simulator
uses `pulsebeam-agent-native` directly.
