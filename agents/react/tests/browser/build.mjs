...
const inputs = ["tests/browser/fixture.tsx", "tests/browser/index.html", "../pulsebeam-agent-web/dist/index.js"];
await writeFile("tests/browser/dist/fixture-manifest.json", JSON.stringify({ inputs: await Promise.all(inputs.map(async (path) => [path, (await stat(path)).mtimeMs])) }));
