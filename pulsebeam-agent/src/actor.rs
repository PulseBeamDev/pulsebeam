...
impl LayerController {
    ...
    fn update_available(&mut self, bw: Bitrate) {
        // apply a simple exponential moving average so that brief transient
        // dips – such as those generated by a local I‑frame burst or by the
        // kernel queue when a pacer is absent – do not immediately drive the
        // allocator into debt.  The raw value is still visible in logs if you
        // turn up tracing, but `available_bps` never falls off a cliff.
        let raw = bw.as_f64();
        if self.available_bps == f64::MAX {
            // first sample: adopt immediately so we don't start with +∞
            self.available_bps = raw;
        } else {
            self.available_bps =
                BW_SMOOTHING_ALPHA * raw + (1.0 - BW_SMOOTHING_ALPHA) * self.available_bps;
        }
    }
