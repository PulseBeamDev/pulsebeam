...
mod tests {
    ...
    async fn test_agent_config_creation() {
        let config = utils::create_test_config("http://localhost:8080/whip");
        assert!(config.audio_enabled);
        assert!(config.video_enabled);
        assert_eq!(config.endpoint, "http://localhost:8080/whip");
    }
    ...
    async fn test_integration_with_real_sfu() {
        let config = utils::create_test_config("http://localhost:7080/whip");
