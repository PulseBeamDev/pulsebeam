...
mod tests {
    ...
    fn test_always_higher_quantization() {
        ...
        assert_eq!(res.as_f64(), 600_000.0);
    }
    ...
    fn test_hysteresis_deadband_prevents_drop() {
        ...
        assert_eq!(ctrl.current().as_f64(), 800_000.0);
    ...
    fn test_hysteresis_deadband_allows_deep_drop() {
        ...
        for _ in 0..40 {
            ...
            if res.as_f64() < 800_000.0 {
                break;
            }
        }

        assert_eq!(res.as_f64(), 400_000.0);
    }
    ...
    fn test_spike_filtering() {
        ...
        assert_eq!(ctrl.current().as_f64(), 400_000.0);

        // A bigger spike should be reflected immediately.
        ctrl.update(Bitrate::kbps(5000));
        assert_eq!(ctrl.current().as_f64(), 5_600_000.0);
        ...
        for _ in 0..50 {
            ...
            if res.as_f64() < 5_600_000.0 {
                break;
            }
        }

        assert_eq!(res.as_f64(), 5_200_000.0);
    }
