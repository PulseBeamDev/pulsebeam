...
pub struct StreamStateInner {
    ...
    // Allocation RTP header extension. `0` means "not declared" — allocation
    // then falls back to the measured `bitrate_bps`. Declared targets are
    // stable (the encoder commits to them), so allocating on them avoids the
    // VBR-driven layer flapping that measured throughput causes.
...
impl StreamStateInner {
    ...
    /// Sender-declared target bitrate (bps) from Video Layers Allocation, or
    /// `0.0` if the sender hasn't declared one. See `declared_target_bps`.
...
impl StreamMonitor {
    ...
    pub fn poll(&mut self, now: Instant, is_any_sibling_active: bool) {
        ...
        let timed_out =
            time_since_last_packet > SIMULCAST_LAYER_PAUSE_TIMEOUT && is_any_sibling_active;
