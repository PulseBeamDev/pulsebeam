...
impl StreamHistory {
    fn check(&mut self, seq: u64, ts: u64) -> Option<EgressViolation> {
...
impl EgressGuard {
    ...
    pub fn check(
        ...
        self.streams.entry((mid, rid)).or_default().check(seq, ts)
    }
}
...
mod test {
    ...
    fn a_clean_stream_reports_nothing() {
        ...
        for frame in 0..50u64 {
            ...
            for _ in 0..3 {
                assert!(guard.check(mid(), None, seq, ts).is_none());
                seq += 1;
            }
        }
    }
    ...
    fn upstream_loss_gaps_are_not_violations() {
        let mut guard = EgressGuard::new();
        assert!(guard.check(mid(), None, 1, 1000).is_none());
        assert!(guard.check(mid(), None, 9, 4000).is_none());
    }
    ...
    fn a_late_packet_carrying_an_older_timestamp_is_not_a_violation() {
        ...
        for frame in 0..20u64 {
            ...
            assert!(guard.check(mid(), None, seq, 1000 + frame * 3000).is_none());
        }
        // It finally arrives, 15 frames late, still carrying its own timestamp.
        assert!(
            guard.check(mid(), None, 5, 1000 + 4 * 3000).is_none(),
    ...
    fn a_sequence_number_carrying_two_different_packets_is_reported() {
        let mut guard = EgressGuard::new();
        assert!(guard.check(mid(), None, 7, 1000).is_none());
        assert!(matches!(
            guard.check(mid(), None, 7, 4000),
    ...
    fn a_packet_the_network_duplicated_is_not_a_violation() {
        let mut guard = EgressGuard::new();
        assert!(guard.check(mid(), None, 7, 1000).is_none());
        assert!(guard.check(mid(), None, 8, 4000).is_none());
        assert!(
            guard.check(mid(), None, 7, 1000).is_none(),
    ...
    fn advancing_the_stream_backwards_in_time_is_reported() {
        let mut guard = EgressGuard::new();
        assert!(guard.check(mid(), None, 1, 4000).is_none());
        assert!(matches!(
            guard.check(mid(), None, 2, 1000),
    ...
    fn streams_are_tracked_independently() {
        ...
        assert!(guard.check(mid(), None, 1, 1000).is_none());
        assert!(guard.check(other, None, 1, 1000).is_none());
    }
}
