...
impl ParticipantActor {
    ...
    async fn poll_and_process_webrtc(
        ...
        self.process_control_effects(ctx, self.core.handle_resync_check(&ctx.handle))
            .await;
    ...
    async fn handle_webrtc_event(&mut self, ctx: &mut actor::ActorContext<Self>, event: Event) {
        match event {
            ...
            Event::MediaData(data) => {
                ...
                if let Some(track_handle) = self.core.get_published_track(data.mid) {
            ...
            // --- CONTROL PLANE ---
            Event::MediaAdded(e) => {
                self.process_control_effects(ctx, self.core.handle_media_added(e))
                    .await;
            }
            ...
            Event::ChannelData(data) if Some(data.id) == self.data_channel_id => {
                self.process_control_effects(ctx, self.core.handle_rpc_data(data))
                    .await;
            }
    ...
    async fn process_control_effects(
        ...
        for effect in effects {
            match effect {
                ...
                ParticipantEffect::RequestKeyframeToTrack(handle, req) => {
                    let _ = handle.try_send_low(track::TrackDataMessage::KeyframeRequest(req));
                }

                ParticipantEffect::SubscribeToTrack(handle, sub) => {
                    let _ = handle
                        .send_high(track::TrackControlMessage::Subscribe(sub))
                        .await;
                }

                ParticipantEffect::SendRpc(bytes) => {
                    if let Some(cid) = self.data_channel_id {
                        if let Some(mut channel) = self.webrtc.channel(cid) {
                ...
                ParticipantEffect::SpawnTrack(meta) => {
                    ...
                    let track_actor = track::TrackActor::new(_ctx.handle.clone(), meta.clone());
