...
impl ShardWorker {
    ...
    async fn run_inner(mut self) -> Result<(), ShardError> {
        loop {
            ...
            // Slow: re-rank Top-N audio selectors for every active room.
            // Each call is rate-limited internally to at most once per 200 ms.
            for (_, room) in &mut self.rooms {
                room.audio_selector.poll_slow(now);
            }
...
///
/// Local delivery: packets are passed through the room's [`TopNAudioSelector`]
/// which updates the per-track EMA and gates delivery to only the top-N
/// loudest streams.  The selector's Top-N set is refreshed every 200 ms by
/// `poll_slow` in the main loop.
fn handle_audio_rtp(
    ev: RtpEvent,
    ...
    // Update the EMA and check whether this track is in the current Top-N.
    let in_top_n = room.audio_selector.push_rtp(ev.stream_id.0, &ev.pkt, now);

    // Cross-shard fanout: only from the originating shard to avoid loops.
    // If the publisher isn't in our local participants map the packet arrived
    // from another shard and must not be re-broadcast.
    ...
    // Local fanout: only deliver if this stream made Top-N.
    if !in_top_n {
        return;
    }
    for &participant_id in &room.members {
        ...
        sub.downstream
            .on_forward_audio_rtp(&ev.stream_id, ev.pkt.clone(), now, &mut sub.rtc);
        dirty.insert(participant_id);
    }
}
