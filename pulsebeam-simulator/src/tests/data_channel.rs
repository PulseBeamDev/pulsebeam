...
#[test]
fn data_channel_route_install_retries_after_failure() {
    LocalNodeSim::new()
        .with_room(
            Room::new("room-data-route-retry")
                .with_participant(Participant::data_participant("pub"))
                .with_participant(Participant::data_participant("sub")),
        )
        .run(vec![
            Step::Run {
                description: "Establish both data participants",
                duration: Duration::from_secs(3),
            },
            Step::DeclarePublishTopic {
            ...
            Step::PublishData {
                description: "Publisher sends after route recovery",
                participant: "pub",
                topic: "retry_topic",
                data: b"route-recovered",
            },
            Step::Run {
                description: "Deliver the recovered stream",
                duration: Duration::from_secs(1),
            },
            Step::CheckDataReceived {
                description: "Subscriber receives after the failed install",
                participant: "sub",
                topic: "retry_topic",
